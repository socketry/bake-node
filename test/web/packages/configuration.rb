...
describe Web::Packages::Configuration do
	...
	it "rejects output outside the project" do
		write_package_json("web-packages" => {"output" => "../public"})
	...
	it "reports malformed configuration" do
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /Could not find/)
		
		File.write(File.join(root, "package.json"), "{")
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /Could not parse/)
		
		write_package_json([])
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /must contain an object/)
		
		write_package_json("web-packages" => [])
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /configuration must be an object/)
		
		write_package_json("dependencies" => [])
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /dependencies must be an object/)
		
		write_package_json("web-packages" => {"packages" => "example"})
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /packages must be an array or object/)
		
		write_package_json("web-packages" => {"base" => "/components"})
		expect do
			subject.load(root)
		end.to raise_exception(Web::Packages::ConfigurationError, message: be =~ /ending in/)
		
		write_package_json("web-packages" => {"packageRoot" => ""})
		expect do
			subject.load(root)
