...
require "bake/node/package"

describe Bake::Node::Package do
	it "validates package names" do
		expect do
			subject.new("../example")
		end.to raise_exception(Bake::Node::ConfigurationError, message: be =~ /Invalid package name/)
	end
	
	it "validates package options" do
		expect do
			subject.new("example", true)
		end.to raise_exception(Bake::Node::ConfigurationError, message: be =~ /must be an object/)
		
		expect do
			subject.new("example", "include" => [])
		end.to raise_exception(Bake::Node::ConfigurationError, message: be =~ /non-empty array/)
		
		expect do
			subject.new("example", "imports" => {"example" => false})
		end.to raise_exception(Bake::Node::ConfigurationError, message: be =~ /map strings/)
	end
